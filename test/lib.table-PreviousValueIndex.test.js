...
test('ADC value equal to the minimal value of the table', () => {
  const index = Table.PreviousValueIndex(TBL_BOSCH_TEMP, 0.038)
  expect(index).toBe(0)
})
...
test('ADC value equal to the maximal value of the table', () => {
  const index = Table.PreviousValueIndex(TBL_BOSCH_TEMP, 0.961)
...
test('ADC value greather than the maximal value of the table', () => {
  const index = Table.PreviousValueIndex(TBL_BOSCH_TEMP, 0.962)
  expect(index).toBe(18)
})
...
test('Typical ADC value equal to a value', () => {
  const index = Table.PreviousValueIndex(TBL_BOSCH_TEMP, 0.152)
  expect(index).toBe(6)
})
